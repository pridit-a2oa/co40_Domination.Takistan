...
0 spawn {
...

    {
        ...
        if ([_allocatable, 0] call BIS_fnc_areEqual) exitWith {};
    ...
    DIALOG("X_PERK_DIALOG", 5) ctrlSetToolTip (
        if (!isNil QMODULE(database) && {!isNil {X_JIP getVariable QGVAR(holiday)}}) then {
            "Happy Holidays! Max points & restrictions lifted until Jan 3rd"
        } else {
           format ["Earn points every %1 score", GVAR(perk_amount_score)]
        }
    );
    ...
    } forEach ([0, GVAR(perk_type_roles)] call FUNC(common,arrayValues));
