...
if (hasInterface) then {
    0 spawn {
        while {true} do {
            ...
            _spawn = 0 spawn {
                {
                    ...
                    if !(isNil "_marker") then {
                        ...
                        if !(isNil QMODULE(vehicle_cargo)) then {
                            [_x, _vehicle] __submodulePP(vehicle_cargo);
                        };

                        if !(isNil QMODULE(vehicle_deploy)) then {
                            [_x, _vehicle] __submodulePP(vehicle_deploy);
                        };

                        if !(isNil QMODULE(vehicle_lift)) then {
                            [_x, _vehicle] __submodulePP(vehicle_lift);
                        };

                        if !(isNil QMODULE(vehicle_wreck)) then {
                            [_x, _vehicle] __submodulePP(vehicle_wreck);
                        };
            ...
            waitUntil {sleep 0.1; scriptDone _spawn};
