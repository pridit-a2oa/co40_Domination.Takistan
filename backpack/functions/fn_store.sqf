...
_action = player addAction [(format ["Equip %1", _name]) call FUNC(common,GreyText), FUNCTION(backpack,equip), [], -7, false, true, "", "player == _target && {(position player) select 2 < 1} && {player getVariable 'd_backpack'} && {count (player getVariable 'd_backpack_hold') > 0} && {(d_backpack_animations find (animationState player)) == -1}"];
