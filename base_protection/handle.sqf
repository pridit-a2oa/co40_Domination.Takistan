...
if (hasInterface) then {
    player addEventHandler ["fired", {
        ...
        switch (true) do {
            case !([{!(true in [_x isKindOf "CAManBase", _x isKindOf "ATV_Base_EP1", _x isKindOf "TT650_Base"]) && {side _x in [west, civilian]}} count nearestObjects [_unit, ["AllVehicles", "ReammoBox"], 20], 0] call BIS_fnc_areEqual);
